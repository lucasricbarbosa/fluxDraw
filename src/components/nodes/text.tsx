...
export function Text({ selected, data }: NodeProps & { data?: TextData }) {
  const [text, setText] = useState("Insira um texto");
  ...
  // Atualiza o texto quando o data.label muda
  useEffect(() => {
    if (data?.label) {
      setText(data.label);
    }
  ...
  return (
    <>
      ...
      <Handle
        ...
        className={`!-right-2 !h-3 !w-3 !border-2 !border-blue-400 !bg-blue-200 ${selected ? "opacity-100" : "opacity-0"}`}
      />
      <Handle
        id="left"
        type="source"
        position={Position.Left}
        className={`!-left-2 !h-3 !w-3 !border-2 !border-blue-400 !bg-blue-200 ${selected ? "opacity-100" : "opacity-0"}`}
      />
      <Handle
        id="top"
        type="source"
        position={Position.Top}
        className={`!-top-2 !h-3 !w-3 !border-2 !border-blue-400 !bg-blue-200 ${selected ? "opacity-100" : "opacity-0"}`}
      />
      <Handle
        id="bottom"
        type="source"
        position={Position.Bottom}
        className={`!-bottom-2 !h-3 !w-3 !border-2 !border-blue-400 !bg-blue-200 ${selected ? "opacity-100" : "opacity-0"}`}
      />
      <div className="flex items-center justify-center">
        {isEditing ? (
          <input
            ...
            className={`flex-1 border-none bg-transparent text-center text-foreground focus:outline-none ${fontSize}`}
          ...
          <ContextMenu>
            <ContextMenuTrigger>
              <p
                className={`cursor-pointer text-foreground ${fontSize} ${fontWeight === "bold" ? "font-bold" : "font-normal"} ${
            ...
            <ContextMenuContent className="w-64">
              ...
              >
                ...
                <ContextMenuRadioItem value="bold">
                  Negrito
                </ContextMenuRadioItem>
              </ContextMenuRadioGroup>
